...
export default async function handler(req: any, res: any) {
  ...
  try {
    // --- 0. Skip non-trading days (weekends/NSE holidays) ---
    const today = new Date().toISOString().slice(0, 10);
    if (!isMarketOpen('IN', today)) {
      console.log(`[IndiaPicks] ${today} is not an India trading day — skipping picks.`);
    ...
    // Run BOTH models in parallel (A/B): new is emailed, old is stored for comparison
    const scoreCfg = {
      market: 'IN' as const,
      quotes: filtered,
      indexChangePercent,
    ...
    const newPicks = await runTwoPassScoring({ ...scoreCfg, model: 'new', logPrefix: '[IndiaPicks:new]' });
    const oldPicks = await runTwoPassScoring({ ...scoreCfg, model: 'old', logPrefix: '[IndiaPicks:old]' });
    ...
    // Save both models' picks to Supabase
    ...
    try {
      ...
      console.log(`[IndiaPicks] Saved ${saved} picks to Supabase (new ${newPicks.length}, old ${oldPicks.length})`);
    } catch (err: any) {
...
<body style="margin:0;padding:0;background-color:#0f172a;color:#e2e8f0;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;">
  <div style="max-width:720px;margin:0 auto;padding:20px;">
    ...
    <div style="margin-bottom:24px;">
      <h2 style="font-size:16px;color:#fbbf24;margin:0 0 12px 0;padding-bottom:8px;border-bottom:1px solid #334155;">👀 Watch List (Score 40–54)</h2>
    ...
    <div style="background:#1e293b;border-radius:8px;padding:16px;margin-bottom:20px;border:1px solid #334155;">
      ...
      <table style="width:100%;font-size:12px;color:#94a3b8;">
        ...
        <tr><td style="padding:2px 0;">Relative strength vs NIFTY</td><td style="text-align:right;">+8</td></tr>
